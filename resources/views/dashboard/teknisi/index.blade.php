...
{{-- ════════ TIKET TEKNISI GROUPED SECTIONS (Airtable / Notion Style) ════════ --}}
                            ...
                            <td style="padding: 8px 14px;">
                                ...
                                @elseif($statusKey === 'proses_service')
                                    <div style="display: flex; gap: 6px; align-items: center;">
                                        <a href="{{ route('dashboard.teknisi.show', $ticket) }}" class="btn btn-outline btn-sm" style="padding: 4px 8px; font-size: 0.72rem;">Detail</a>
                                        <form action="{{ route('dashboard.teknisi.update', $ticket) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin servis unit ini sudah SELESAI? Status tiket akan diubah menjadi Siap Diambil.');" style="margin: 0;">
                                            @csrf
                                            <input type="hidden" name="action" value="done">
                                            <button type="submit" class="btn btn-success btn-sm" style="padding: 4px 10px; font-size: 0.72rem; font-weight: 800; background: #16a34a; border-color: #16a34a; color: #fff;">
                                                Selesai Servis
                                            </button>
                                        </form>
                                    </div>
                                @else
                                    <a href="{{ route('dashboard.teknisi.show', $ticket) }}" class="btn btn-primary btn-sm" style="padding: 5px 12px; font-weight:700;">Detail & Aksi</a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
